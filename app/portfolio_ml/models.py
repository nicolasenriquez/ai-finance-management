...
class PortfolioMLModelSnapshot(Base, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint("snapshot_ref", name="uq_portfolio_ml_model_snapshot_ref"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    snapshot_ref: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
    scope: Mapped[str] = mapped_column(String(32), nullable=False, index=True)
    instrument_symbol: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    ...
    replaced_snapshot_ref: Mapped[str | None] = mapped_column(String(128), nullable=True)
    ...
    baseline_comparator_metrics: Mapped[dict[str, object]] = mapped_column(JSON, nullable=False)
    failure_reason_code: Mapped[str | None] = mapped_column(String(64), nullable=True)
    failure_reason_detail: Mapped[str | None] = mapped_column(String(255), nullable=True)
