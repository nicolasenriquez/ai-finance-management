...
class PdfExtractedRow(BaseModel):
    ...
    row_id: int = Field(ge=1, description="Deterministic row identifier in emission order.")
    source_page: int = Field(ge=1, description="Source PDF page where the row originated.")
...
class PdfExtractedTable(BaseModel):
    ...
    table_name: str = Field(min_length=1, description="Logical extracted table identifier.")
    columns: list[str] = Field(
        min_length=1,
        description="Deterministic output column order for this extracted table.",
    )
    rows: list[PdfExtractedRow] = Field(description="Row-wise extracted payload for the table.")
...
class PdfExtractionResult(BaseModel):
    ...
    storage_key: str = Field(min_length=1, description="Storage key used for this extraction run.")
