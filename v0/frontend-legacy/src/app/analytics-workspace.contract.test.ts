...
type WorkspaceRouteContract = {
  ...
  requiresHeroInsight: boolean;
};

const workspaceContracts: WorkspaceRouteContract[] = [
  {
    routeName: "Overview/Home",
    ...
    requiresHeroInsight: true,
  },
  {
    routeName: "Holdings",
    path: "/portfolio/holdings",
    pageFile: "src/pages/portfolio-holdings-page/PortfolioHoldingsPage.tsx",
    testFile: "src/pages/portfolio-holdings-page/PortfolioHoldingsPage.test.tsx",
    requiresPrimaryJob: true,
    requiresHeroInsight: true,
  },
  {
...
describe("analytics workspace route contract", () => {
  it("registers canonical workspace paths while preserving existing portfolio routes", () => {
    ...
    expect(
      missingPaths,
      "Fail-first baseline: add Dashboard/Holdings/Performance/Transactions routes in app router.",
    ).toEqual([]);
  });
  ...
  it.each(workspaceContracts)(
    ...
    ({
      ...
      if (requiresPrimaryJob) {
        ...
        expect(
          pageSource.includes('viewportRole="dominant-job"'),
          `Fail-first baseline: ${routeName} page must declare one dominant-job viewport role.`,
        ).toBe(true);
      }
      if (requiresHeroInsight) {
        expect(
          pageSource.includes('viewportRole="hero-insight"'),
          `Fail-first baseline: ${routeName} page must declare one hero insight viewport role.`,
        ).toBe(true);
        expect(
          pageSource.includes('hierarchy="hero"'),
          `Fail-first baseline: ${routeName} hero insight must use hero hierarchy.`,
        ).toBe(true);
      }
