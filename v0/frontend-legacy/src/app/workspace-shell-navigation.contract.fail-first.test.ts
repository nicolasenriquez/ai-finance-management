...
import { resolveWorkspaceShellDensityModeForPath } from "../features/portfolio-workspace/dashboard-governance";
...
describe("workspace shell navigation fail-first contract", () => {
  ...
  it("2.1 enforces route-aware shell density profiles as expanded/standard/compact", () => {
    expect(resolveWorkspaceShellDensityModeForPath("/portfolio/dashboard")).toBe("expanded");
    expect(resolveWorkspaceShellDensityModeForPath("/portfolio/holdings")).toBe("standard");
    expect(resolveWorkspaceShellDensityModeForPath("/portfolio/performance")).toBe(
      "standard",
    );
  ...
  it("2.3 keeps freshness/provenance trust metadata in a support rail with explicit lens orientation copy", () => {
    const layoutPath = resolve(
      process.cwd(),
      "src/components/workspace-layout/PortfolioWorkspaceLayout.tsx",
    );
    ...
    const layoutSource = readFileSync(layoutPath, "utf8").toLowerCase();
    expect(
      layoutSource.includes("workspace-support-rail"),
      "Fail-first baseline: shell must render compact support rail for trust metadata placement.",
    ).toBe(true);
    expect(
      layoutSource.includes("active lens orientation"),
      "Fail-first baseline: shell must expose explicit active lens orientation copy.",
    ).toBe(true);
  });
});
