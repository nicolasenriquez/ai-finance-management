...
function slugifyLensLabel(lensLabel: string | null): string {
  if (!lensLabel) {
    return "workspace";
  }
  const normalized = lensLabel.trim().toLowerCase();
...
export function PortfolioWorkspaceLayout({
  ...
  const activeLensSlug = slugifyLensLabel(routeLensLabel);
  ...
  return (
    ...
    >
      <section
        ...
        } workspace-layout--lens-${activeLensSlug}`}
        data-workspace-lens={activeLensSlug}
        data-shell-density-mode={resolvedShellDensityMode}
      >
        <nav
          className={
            layoutVariant === "chat"
              ? "panel workspace-panel workspace-panel--utility workspace-nav workspace-nav--chat"
              : "panel workspace-panel workspace-panel--utility workspace-nav"
          }
        ...
        >
                ...
                {(() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        aria-hidden="true"
                        className={`workspace-nav__route-lens workspace-nav__route-lens--${slugifyLensLabel(route.lens)}`}
                      >
                        {route.lens}
                      </span>
                    </NavLink>
          ...
          <p className="workspace-nav__lens-copy">
            Active lens orientation: {routeLensLabel || "Workspace"}
          </p>
        </nav>
        ...
        {resolvedShowTrustPanel ? (
          <section
            className="panel workspace-panel workspace-panel--utility workspace-trust workspace-support-rail"
            aria-label="Data trust context"
          >
            <header className="workspace-support-rail__header">
              <h2 className="workspace-support-rail__title">Route support rail</h2>
              <p className="workspace-support-rail__copy">
                Freshness, provenance, and scope remain persistent support context.
              </p>
            </header>
