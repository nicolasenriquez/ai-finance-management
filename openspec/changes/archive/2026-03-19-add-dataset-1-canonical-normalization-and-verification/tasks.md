...
Notes: Updated `docs/guides/pdf-extraction-guide.md` to mark canonical normalization (`/pdf/normalize`) and verification (`/pdf/verify`) as implemented downstream slices, while keeping persistence and deduplication explicitly pending.
Notes: Updated `docs/guides/validation-baseline.md` to reflect the current dataset 1 canonical baseline (extraction + normalization + verification implemented; persistence validation pending).
